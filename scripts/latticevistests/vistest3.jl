...
# Curve 2: anyon(tile 8) → e2e(tile 5) → anyon(tile 9) → e2e(tile 6) →
#          anyon(tile 3) → e2e(tile 5) → anyon(tile 2)
#
# Segment 1 (anyon_count=1): tiles 8, 5, 9(enter)
#   tile 8 edge 3 (SE) ↔ tile 5 edge 6 (NW)
#   tile 5 edge 1 (NE) ↔ tile 9 edge 4 (SW)
# Segment 2 (anyon_count=2): tiles 9(exit), 6, 3(enter)
#   tile 9 edge 3 (SE) ↔ tile 6 edge 6 (NW)
#   tile 6 edge 4 (SW) ↔ tile 3 edge 1 (NE)
# Segment 3 (anyon_count=3): tiles 3(exit), 5, 2
...
cp9a  = insert_curvepiece!(get_tile(l, 9), curve2, 1, 4, 1, IN)        # e2a, seg 1
cp9b  = insert_curvepiece!(get_tile(l, 9), curve2, 2, 3, 1, OUT)       # a2e, seg 2
cp6b  = insert_curvepiece!(get_tile(l, 6), curve2, 2, 6, 1, 4, 1)      # e2e, seg 2
cp3a  = insert_curvepiece!(get_tile(l, 3), curve2, 2, 1, 1, IN)        # e2a, seg 2
cp3b  = insert_curvepiece!(get_tile(l, 3), curve2, 3, 6, 1, OUT)       # a2e, seg 3
cp5c  = insert_curvepiece!(get_tile(l, 5), curve2, 3, 3, 1, 4, 1)      # e2e, seg 3
cp2   = insert_curvepiece!(get_tile(l, 2), curve2, 3, 1, 1, IN)        # e2a, seg 3

CurveDiagrams._insert_cref!(l, curve2, 1, CurvepieceRef(8, cp8))
CurveDiagrams._insert_cref!(l, curve2, 2, CurvepieceRef(5, cp5b))
CurveDiagrams._insert_cref!(l, curve2, 3, CurvepieceRef(9, cp9a))
CurveDiagrams._insert_cref!(l, curve2, 4, CurvepieceRef(9, cp9b))
CurveDiagrams._insert_cref!(l, curve2, 5, CurvepieceRef(6, cp6b))
CurveDiagrams._insert_cref!(l, curve2, 6, CurvepieceRef(3, cp3a))
CurveDiagrams._insert_cref!(l, curve2, 7, CurvepieceRef(3, cp3b))
CurveDiagrams._insert_cref!(l, curve2, 8, CurvepieceRef(5, cp5c))
CurveDiagrams._insert_cref!(l, curve2, 9, CurvepieceRef(2, cp2))
